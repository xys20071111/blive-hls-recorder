...
function parseStreamListM3U8(list: string): string {
  let isStreamFind = false;
  ...
  throw new Error("Couldn't find stream!")
}
...
export function startM3u8Recording(
  ctx: M3u8RecorderContext,
  ...
  return setInterval(async () => {
    ...
    try {
      ...
      if (m3u8Text.includes("#EXT-X-STREAM-INF")) {
        const targetUrl = parseStreamListM3U8(m3u8Text)
        m3u8Text = await (
